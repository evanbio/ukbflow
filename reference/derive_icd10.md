...
derive_icd10(
  ...
  primary_cols = NULL,
  secondary_cols = NULL,
  death_cause = c("any", "primary", "secondary"),
...
- death_cause:
  ...
  (character) Passed as `cause` to
  [`derive_death_registry`](https://evanbio.github.io/ukbflow/reference/derive_death_registry.md):
  `"any"` (default), `"primary"` (underlying cause only), or
  `"secondary"` (contributory causes only).
...
The input `data` (invisibly) with `{name}_icd10` (logical) and
`{name}_icd10_date` (IDate) added in-place, plus all intermediate source
...
#> ✔ derive_first_occurrence (mi): 13 cases with valid date.
#> ✔ derive_icd10 (mi): 15 cases across 3 sources, 15 with date.
```
