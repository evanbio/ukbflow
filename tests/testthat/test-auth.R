# test-auth.R
# Unit tests for auth_ series functions (mock-based, no network required)
# -------------------------------------------------------------------------
...
test_that("auth_login() throws error when token is an empty string and env var is unset", {
  withr::with_envvar(c(DX_API_TOKEN = ""), {
    expect_error(auth_login(token = ""), "DX_API_TOKEN")
  })
})
...
test_that("auth_status() throws error when not logged in", {
  mockery::stub(auth_status, ".dx_run", .fake_dx(stderr = "Not logged in", status = 1))
...
test_that("auth_status() returns NA project when no project is selected", {
  ...
  mockery::stub(auth_status, ".dx_get_project_id", NA_character_)
...
test_that("auth_status() returns correct user and project when logged in", {
  mockery::stub(auth_status, ".dx_run", function(args, ...) {
    .fake_dx(stdout = "TestUser")
  })
  mockery::stub(auth_status, ".dx_get_project_id", "project-XXXXXXXXXXXX")
  result <- suppressMessages(auth_status())
...
test_that("auth_logout() throws error when dx logout fails", {
  mockery::stub(auth_logout, ".dx_run", .fake_dx(stderr = "Not logged in", status = 1))
  expect_error(auth_logout(), "Logout failed")
})

test_that("auth_logout() returns TRUE invisibly on success", {
  mockery::stub(auth_logout, ".dx_run", .fake_dx(status = 0))
...
test_that("auth_list_projects() throws error when dx find fails", {
  mockery::stub(auth_list_projects, ".dx_run", .fake_dx(stderr = "Auth error", status = 1))
...
test_that("auth_list_projects() returns empty vector when no projects found", {
  mockery::stub(auth_list_projects, ".dx_run", .fake_dx(stdout = "", status = 0))
...
test_that("auth_list_projects() returns character vector of projects", {
  ...
  mockery::stub(auth_list_projects, ".dx_run", .fake_dx(stdout = fake_output, status = 0))
  result <- suppressMessages(auth_list_projects())
...
test_that("auth_select_project() throws error when dx select fails", {
  mockery::stub(auth_select_project, ".dx_run", .fake_dx(stderr = "Not found", status = 1))
  expect_error(auth_select_project("project-XXXXXXXXXXXX"), "Failed to select project")
})

test_that("auth_select_project() throws error when project context is not updated", {
  mockery::stub(auth_select_project, ".dx_run", .fake_dx(status = 0))
  mockery::stub(auth_select_project, ".dx_get_project_id", NA_character_)
...
test_that("auth_select_project() returns TRUE invisibly on success", {
  mockery::stub(auth_select_project, ".dx_run", .fake_dx(status = 0))
  mockery::stub(auth_select_project, ".dx_get_project_id", "project-XXXXXXXXXXXX")
