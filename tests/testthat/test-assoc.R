...
.fake_assoc_dt <- function(n = 300, seed = 42) {
  ...
  dt <- data.table::data.table(
    ...
    cscc_01            = rbinom(n, 1, 0.10),
    ad_01              = rbinom(n, 1, 0.20),
    ad_tf              = as.logical(rbinom(n, 1, 0.20)),
    bmi_cat            = factor(
...
test_that("assoc_coxph() aborts on non-data.frame input", {
  expect_error(
    assoc_coxph("not a df", "cscc_01", "followup_years", "ad_01"),
    "data.frame"
  )
})
...
test_that("assoc_coxph() aborts on missing column", {
  dt <- .fake_assoc_dt()
  expect_error(
    suppressMessages(assoc_coxph(dt, "nonexistent", "followup_years", "ad_01")),
    "not found"
  )
})
...
test_that("assoc_coxph() aborts when base=FALSE and no covariates", {
  dt <- .fake_assoc_dt()
  expect_error(
    suppressMessages(assoc_coxph(dt, "cscc_01", "followup_years", "ad_01",
                                 base = FALSE)),
    "covariates"
  )
})
...
test_that("assoc_coxph() binary term name matches exposure name (no suffix)", {
  ...
  expect_true(all(res$term == "ad_01"))
})

test_that("assoc_coxph() logical exposure converts to integer (no TRUE suffix)", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(
    assoc_coxph(dt, "cscc_01", "followup_years", "ad_tf")
  )
  expect_true(all(res$term == "ad_tf"))
})
...
test_that("assoc_coxph() multiple exposures stack rows correctly", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(
    assoc_coxph(dt, "cscc_01", "followup_years", c("ad_01", "bmi_num"))
  )
...
test_that("assoc_cox() alias matches assoc_coxph()", {
  dt   <- .fake_assoc_dt()
  res1 <- suppressMessages(assoc_coxph(dt, "cscc_01", "followup_years", "ad_01"))
  res2 <- suppressMessages(assoc_cox(dt,   "cscc_01", "followup_years", "ad_01"))
...
test_that("assoc_logistic() aborts on non-data.frame input", {
  expect_error(
    assoc_logistic("not a df", "cscc_01", "ad_01"),
    "data.frame"
  )
})
...
test_that("assoc_logistic() aborts on missing column", {
  dt <- .fake_assoc_dt()
  expect_error(
    suppressMessages(assoc_logistic(dt, "nonexistent", "ad_01")),
    "not found"
  )
})
...
test_that("assoc_logistic() returns data.table with OR columns", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(assoc_logistic(dt, "cscc_01", "ad_01"))
  expect_true(data.table::is.data.table(res))
...
test_that("assoc_logistic() aborts on non-binary outcome", {
  ...
  expect_error(
    suppressMessages(assoc_logistic(dt, "bmi_int", "ad_01")),
    "0/1"
  )
})
...
test_that("assoc_logit() alias matches assoc_logistic()", {
  dt   <- .fake_assoc_dt()
  res1 <- suppressMessages(assoc_logistic(dt, "cscc_01", "ad_01"))
  res2 <- suppressMessages(assoc_logit(dt,    "cscc_01", "ad_01"))
...
test_that("assoc_linear() aborts on non-data.frame input", {
  expect_error(
    assoc_linear("not a df", "bmi_num", "ad_01"),
    "data.frame"
  )
})
...
test_that("assoc_linear() returns data.table with beta/se columns", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(assoc_linear(dt, "bmi_num", "ad_01"))
  expect_true(data.table::is.data.table(res))
...
test_that("assoc_lm() alias matches assoc_linear()", {
  dt   <- .fake_assoc_dt()
  res1 <- suppressMessages(assoc_linear(dt, "bmi_num", "ad_01"))
  res2 <- suppressMessages(assoc_lm(dt,     "bmi_num", "ad_01"))
...
test_that("assoc_coxph_zph() aborts on missing column", {
  ...
  expect_error(
    suppressMessages(
      assoc_coxph_zph(dt, "nonexistent", "followup_years", "ad_01")
    ),
    "not found"
  )
})
...
test_that("assoc_zph() alias matches assoc_coxph_zph()", {
  dt   <- .fake_assoc_dt()
  res1 <- suppressMessages(assoc_coxph_zph(dt, "cscc_01", "followup_years", "ad_01"))
  res2 <- suppressMessages(assoc_zph(dt,        "cscc_01", "followup_years", "ad_01"))
...
test_that("assoc_subgroup() aborts on missing by column", {
  ...
  expect_error(
    suppressMessages(
      assoc_subgroup(dt, "cscc_01", "followup_years", "ad_01", by = "nonexistent")
    ),
    "not found"
  )
})
...
test_that("assoc_sub() alias matches assoc_subgroup()", {
  ...
  res2 <- suppressMessages(
    assoc_sub(dt, "cscc_01", "followup_years", "ad_01", by = "sex"))
  expect_equal(res1$HR, res2$HR)
})
...
test_that("assoc_trend() aborts when exposure is not factor", {
  ...
  expect_error(
    suppressMessages(
      assoc_trend(dt, "cscc_01", "followup_years", "ad_01", method = "coxph")
    ),
    "factor"
  )
})
...
test_that("assoc_trend() aborts when scores length != nlevels", {
  ...
  expect_error(
    suppressMessages(
      assoc_trend(dt, "cscc_01", "followup_years", "bmi_cat",
...
test_that("assoc_trend() returns data.table with level and p_trend columns", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(
    assoc_trend(dt, "cscc_01", "followup_years", "bmi_cat", method = "coxph")
  )
  expect_true(data.table::is.data.table(res))
...
test_that("assoc_tr() alias matches assoc_trend()", {
  ...
  res1 <- suppressMessages(
    assoc_trend(dt, "cscc_01", "followup_years", "bmi_cat", method = "coxph"))
  res2 <- suppressMessages(
    assoc_tr(dt,   "cscc_01", "followup_years", "bmi_cat", method = "coxph"))
  expect_equal(res1$HR, res2$HR)
})
...
test_that("assoc_competing() aborts on missing column", {
  ...
  expect_error(
    suppressMessages(
      assoc_competing(dt, "nonexistent", "followup_years", "ad_01")
    ),
    "not found"
  )
})
...
test_that("assoc_competing() logical exposure normalised (no TRUE suffix in term)", {
  dt  <- .fake_competing_dt()
  res <- suppressMessages(
    assoc_competing(dt, "censoring_type", "followup_years", "ad_tf",
                    event_val = 1L, compete_val = 2L)
  )
  expect_true(all(res$term == "ad_tf"))
})
...
test_that("assoc_fg() alias matches assoc_competing()", {
  ...
  res1 <- suppressMessages(
    assoc_competing(dt, "censoring_type", "followup_years", "ad_01",
                    event_val = 1L, compete_val = 2L))
  res2 <- suppressMessages(
    assoc_fg(dt, "censoring_type", "followup_years", "ad_01",
             event_val = 1L, compete_val = 2L))
...
test_that("assoc_lag() aborts on missing column", {
  ...
  expect_error(
    suppressMessages(
      assoc_lag(dt, "nonexistent", "followup_years", "ad_01")
    ),
    "not found"
  )
})
...
test_that("assoc_lag() aborts on negative lag_years", {
  ...
  expect_error(
    suppressMessages(
      assoc_lag(dt, "cscc_01", "followup_years", "ad_01", lag_years = c(-1, 2))
    ),
    "non-negative"
  )
})
...
test_that("assoc_lag() lag=0 excludes nobody (n_excluded = 0)", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(
    assoc_lag(dt, "cscc_01", "followup_years", "ad_01", lag_years = c(0, 2))
  )
...
test_that("assoc_lag() larger lag excludes more participants", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(
    assoc_lag(dt, "cscc_01", "followup_years", "ad_01", lag_years = c(2, 5))
  )
...
test_that("assoc_lag() lag_years and n_excluded placed after model column", {
  dt  <- .fake_assoc_dt()
  res <- suppressMessages(
    assoc_lag(dt, "cscc_01", "followup_years", "ad_01", lag_years = 1)
  )
