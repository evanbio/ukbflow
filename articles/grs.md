...
validating polygenic / genetic risk scores within the UK Biobank
Research Analysis Platform (RAP). Because individual-level genotype data
...
> **Prerequisite**: you must be authenticated to UKB RAP and have a
...
Before uploading to RAP, validate your SNP weights file. The function
reads the file, checks required columns, flags problems, and writes a
...
the plink2-native PGEN format with a MAF filter applied via plink2. This
staged workflow separates one-time genotype conversion from repeated GRS
scoring, so the converted PGEN files can be reused across multiple score
files.

The trade-off is additional RAP project storage: PGEN outputs should be
kept only when they will be reused, and removed according to the
project’s storage and cost policy when no longer needed.
...
Once PGEN files are ready,
[`grs_score()`](https://evanbio.github.io/ukbflow/reference/grs_score.md)
uploads your weights file(s) and submits one plink2 scoring job per GRS.
...
> [`grs_bgen2pgen()`](https://evanbio.github.io/ukbflow/reference/grs_bgen2pgen.md)
> so that the correct PGEN files are located.

Output per job: `/grs/<GRS_name>_scores.csv` with columns `IID` and the
GRS score named `GRS_<name>`.
...
is an alias and produces an identical result.

The original columns are preserved; `_z` columns are inserted
immediately after their source column.
...
runs four complementary validation analyses for each GRS:
...
- [`?grs_standardize`](https://evanbio.github.io/ukbflow/reference/grs_standardize.md),
  [`?grs_validate`](https://evanbio.github.io/ukbflow/reference/grs_validate.md)
- [`vignette("auth")`](https://evanbio.github.io/ukbflow/articles/auth.md)
  – RAP authentication and project selection
- [`vignette("job")`](https://evanbio.github.io/ukbflow/articles/job.md)
  – monitoring and managing RAP jobs
- [`vignette("assoc")`](https://evanbio.github.io/ukbflow/articles/assoc.md)
  – association analysis functions used inside
  [`grs_validate()`](https://evanbio.github.io/ukbflow/reference/grs_validate.md)
