...
`ukbflow` is an R package for UK Biobank data analysis and visualization, designed to work natively with the [UK Biobank Research Analysis Platform (RAP)](https://ukbiobank.dnanexus.com). It supports two working modes:

- **Local → RAP**: Run R locally, authenticate with DNAnexus, submit jobs, and download summary results.
- **RAP → RAP**: Run R directly inside the RAP cloud environment, where authentication is handled automatically.
...
- **dxpy**: Required for local → RAP authentication and job submission (see below)
...
- **processx** — Reliable system command execution with robust error handling (used by auth and job functions)
...
The `auth_*` and `job_*` functions rely on the `dx` command-line tool from [dxpy](https://documentation.dnanexus.com/downloads). Required only when running locally.
...
> For full details on token management, project selection, and both authentication modes, see `vignette("authentication")`.
...
**Solution**: Install [Rtools](https://cran.r-project.org/bin/windows/Rtools/) for packages that require compilation.
...
1. Read the [Getting Started](getting-started.html) guide
2. Browse the [Function Reference](https://evanbio.github.io/ukbflow/reference/)
