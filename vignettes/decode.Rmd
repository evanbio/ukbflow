...
| `extract_batch()` | `p31`, `p53_i0` | Already decoded — skip `decode_values()` |
...
`decode_values()` converts raw integer codes to human-readable labels for all categorical fields. Continuous, date, and text fields are left unchanged.
...
`decode_names()` renames columns from field ID format to snake_case labels using the UKB field title dictionary.
...
Some UKB field titles are verbose. Names exceeding 60 characters are flagged with a warning:
